...
def GHAC(mlist, papers, n_clusters=-1):
    ...
    # Crie uma lista de nós conectados
    connected_nodes = [node for node in range(len(mlist)) if any(paper_weight[node])]

    # Crie submatrizes com base nos nós conectados
    paper_weight_sub = np.array(PHNet.loc[papers][papers])
    paper_weight_sub = paper_weight_sub[connected_nodes][:, connected_nodes]
    mlist_sub = [mlist[node] for node in connected_nodes]

    distance = []
    graph = []

    for i in range(len(mlist_sub)):
        gtmp = []
        for j in range(len(mlist_sub)):
            if i < j and paper_weight_sub[i][j] != 0:
                cosdis = np.dot(mlist_sub[i], mlist_sub[j]) / (np.linalg.norm(mlist_sub[i]) * (np.linalg.norm(mlist_sub[j])))
                gtmp.append(cosdis * paper_weight_sub[i][j])
            elif i > j:
