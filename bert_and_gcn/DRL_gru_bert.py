...
num_step = 20
word_input = 768  # tamanho do vetor de embedding BERT base
...
for paperid in paperid_title:
    split_cut = tokenizer(paperid_title[paperid], padding='max_length', truncation=True, max_length=num_step, return_tensors="pt")
    with torch.no_grad():
        outputs = model(**split_cut)
    words_vec = outputs.last_hidden_state
    words_vec = words_vec.detach().numpy().squeeze(0)
    lenth = np.where(split_cut["attention_mask"].numpy()[0] == 1)[0].shape[0]
...
def batch_gen(X,X_len,Y,Y1,batch_size):  # batch generator
    idx = 0
    while True:
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        ...
        _,loss_val,acc_val = session.run([optimizer,loss,accuracy],feed_dict=feed_dict)
    ...
    paper_final_vec, paper_final_pred, paper_final_predj = session.run([embed,pred,predj], feed_dict={train_inputs: paper1,seq_lengths:paper_lens1})

    with open('gene/gruemb.pkl', "wb") as file_obj:
        pickle.dump(paper_final_vec, file_obj)
