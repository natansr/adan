...
class GHAC:
    ...
    def cluster_evaluate(embedding_path, json_dir, result_box):
        ...
        start_msg = f"🟢 Início da validação: {start_time.strftime('%Y-%m-%d %H:%M:%S')}"
        print(start_msg)
        result_box.insert("end", start_msg + "\n")
        result_box.see("end")
        ...
        process = psutil.Process(os.getpid())  # Para uso do psutil
        mem_logged = False
        total_files = len(file_names)

        for idx, fname in enumerate(tqdm(file_names, desc="Processing JSON files")):
            ...
            # Medir memória na metade dos arquivos
            if not mem_logged and (idx + 1) >= total_files // 2:
                mem_usage_mb = process.memory_info().rss / (1024 ** 2)
                mem_msg = f"🧠 Memória utilizada na metade da execução: {mem_usage_mb:.2f} MB"
                print(mem_msg)
                result_box.insert("end", mem_msg + "\n")
                result_box.see("end")
                mem_logged = True
        ...
        end_msg = f"✅ Fim da validação: {end_time.strftime('%Y-%m-%d %H:%M:%S')}"
        duration_msg = f"⏱️ Duração total: {(end_time - start_time).total_seconds():.2f} segundos"
        print(end_msg)
        print(duration_msg)
        result_box.insert("end", end_msg + "\n" + duration_msg + "\n")
        result_box.see("end")
