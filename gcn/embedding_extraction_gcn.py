...
import psutil  # ⬅️ Adicionado para monitorar memória
...
def train_gcn(data, input_dim, num_layers, epochs, progress_bar, progress_label, output_box):
    ...
    start_msg = f"🟢 Treinamento iniciado em: {start_time.strftime('%Y-%m-%d %H:%M:%S')}"
    print(start_msg)
    output_box.insert("end", start_msg + "\n")
    output_box.see("end")
    ...
    for epoch in range(epochs):
        ...
        if not memory_logged and epoch + 1 == epochs // 2:
            mem_usage_mb = process.memory_info().rss / (1024 ** 2)
            mem_msg = f"🧠 Memória utilizada na metade das épocas: {mem_usage_mb:.2f} MB"
            print(mem_msg)
            output_box.insert("end", mem_msg + "\n")
            output_box.see("end")
            memory_logged = True

    end_time = datetime.now()
    end_msg = f"✅ Treinamento finalizado em: {end_time.strftime('%Y-%m-%d %H:%M:%S')}"
    duration = (end_time - start_time).total_seconds()
    duration_msg = f"⏱️ Duração total: {duration:.2f} segundos"

    print(end_msg)
    print(duration_msg)
    output_box.insert("end", end_msg + "\n" + duration_msg + "\n")
    output_box.see("end")
